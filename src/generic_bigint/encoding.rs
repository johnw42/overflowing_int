...
use crate::{BigNumber, small_num::SmallNumber};
    ...
    B: From<S>,
{
    ...
    fn into_big_cow(self) -> Cow<'a, B> {
        match self.decode() {
            Decoded::Small(s) => Cow::Owned(B::from(s)),
            Decoded::Big(b) => Cow::Owned(b.into_owned()),
        }
    }
}
...
where
    Self: Sized + Clone,
{
    type Small: SmallNumber;
    type SmallUint: SmallNumber + Unsigned + TryFrom<Self::Big>;
    type Big: BigNumber + From<Self::Small>;
