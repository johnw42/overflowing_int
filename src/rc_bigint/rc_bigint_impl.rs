...
mod tests {
    ...
    type RcBigInt = GenericBigInt<'static, E>;
    ...
    fn prop_from_signed_bytes_be_matches_bigint(bytes: Vec<u8>) {
        assert_eq!(
            E::Big::from(GenericBigInt::from_signed_bytes_be(&bytes)),
    ...
    fn prop_from_signed_bytes_le_matches_bigint(bytes: Vec<u8>) -> bool {
        E::Big::from(GenericBigInt::from_signed_bytes_le(&bytes))
            == E::Big::from_signed_bytes_le(&bytes)
    }

    #[quickcheck]
    fn prop_to_bytes_be_matches_bigint(n: GenericBigInt) -> bool {
        E::Big::from(&n).to_bytes_be() == n.to_bytes_be()
    }

    #[quickcheck]
    fn prop_to_bytes_le_matches_bigint(n: GenericBigInt) -> bool {
        E::Big::from(&n).to_bytes_le() == n.to_bytes_le()
    }

    #[quickcheck]
    fn prop_to_signed_bytes_be_matches_bigint(n: GenericBigInt) -> bool {
        E::Big::from(&n).to_signed_bytes_be() == n.to_signed_bytes_be()
    }

    #[quickcheck]
    fn prop_to_signed_bytes_le_matches_bigint(n: GenericBigInt) -> bool {
        E::Big::from(&n).to_signed_bytes_le() == n.to_signed_bytes_le()
    }

    #[quickcheck]
    fn prop_sign_matches_bigint(n: GenericBigInt) -> bool {
        E::Big::from(&n).sign() == n.sign()
    }

    #[quickcheck]
    fn prop_bit_matches_bigint(n: GenericBigInt, bit: u64) {
        let bit = bit % 1024;
        assert_eq!(E::Big::from(n.clone()).bit(bit), n.bit(bit));
    }

    #[quickcheck]
    fn prop_bits_matches_bigint(n: GenericBigInt) -> bool {
        E::Big::from(&n).bits() == n.bits()
    }

    #[quickcheck]
    fn prop_to_biguint_matches_bigint(n: GenericBigInt) -> bool {
        E::Big::from(&n).to_biguint() == n.to_biguint()
    }

    #[quickcheck]
    fn prop_checked_add_matches_bigint(n1: GenericBigInt, n2: GenericBigInt) -> bool {
        n1.checked_add(&n2).into() == E::Big::from(&n1).checked_add(&E::Big::from(&n2))
    }

    #[quickcheck]
    fn prop_checked_sub_matches_bigint(n1: GenericBigInt, n2: GenericBigInt) -> bool {
        n1.checked_sub(&n2).into() == E::Big::from(&n1).checked_sub(&E::Big::from(&n2))
    }

    #[quickcheck]
    fn prop_checked_mul_matches_bigint(n1: GenericBigInt, n2: GenericBigInt) -> bool {
        n1.checked_mul(&n2).into() == E::Big::from(&n1).checked_mul(&E::Big::from(&n2))
    }

    #[quickcheck]
    fn prop_checked_div_matches_bigint(n1: GenericBigInt, n2: GenericBigInt) -> bool {
        n1.checked_div(&n2).into() == E::Big::from(&n1).checked_div(&E::Big::from(&n2))
    }

    #[quickcheck]
    fn prop_pow_matches_bigint(n: GenericBigInt, exp: u32) -> bool {
        let k = exp % 16;
    ...
    #[quickcheck]
    fn prop_sqrt_matches_bigint(n: GenericBigInt) -> bool {
        E::Big::from(&n).sqrt() == n.sqrt().into()
    }

    #[quickcheck]
    fn prop_cbrt_matches_bigint(n: GenericBigInt) -> bool {
        E::Big::from(&n).cbrt() == n.cbrt().into()
    }

    #[quickcheck]
    fn prop_nth_root_matches_bigint(n: GenericBigInt, k: u32) -> bool {
        let k = k % 16 + 1;
        E::Big::from(&n).nth_root(k) == n.nth_root(k).into()
    }

    #[quickcheck]
    fn prop_to_u32_digits_matches_bigint(n: GenericBigInt) -> bool {
        E::Big::from(&n).to_u32_digits().eq(&mut n.to_u32_digits())
    }

    #[quickcheck]
    fn prop_to_u64_digits_matches_bigint(n: GenericBigInt) -> bool {
        E::Big::from(&n).to_u64_digits().eq(&mut n.to_u64_digits())
    }

    #[quickcheck]
    fn prop_modinv_matches_bigint(n: GenericBigInt, modulus: GenericBigInt) -> bool {
        E::Big::from(&n).modinv(&E::Big::from(&modulus)) == n.modinv(&modulus).map(E::Big::from)
    }

    #[quickcheck]
    fn prop_trailing_zeros_matches_bigint(n: GenericBigInt) -> bool {
        E::Big::from(&n).trailing_zeros() == n.trailing_zeros()
    }

    #[quickcheck]
    fn prop_set_bit_matches_bigint(mut n1: GenericBigInt, bit: u64, value: bool) -> bool {
